...
uses additional http headers to tell browser whether web app can share resource with another web app
(don't have same origin)

Browsers never allowed web app to share resource with other different origin web apps.
 But now with CORS Standard we can achieve tis
...
We need to pass an additional header from source to target.
...
Wildcard character (*) means that any site can access the resource you have it in your site 
and obviously its unsafe.

Based on the request methods (GET/PUT/POST/DELETE) and the request headers,
 the requests are classified into two categories.

 ** Simple Requests **-- simple a request is made to 3rd party with origin header.
